...
class Animation:
    ...
    def legend(self):
        ...
        if self.agent_pos == self.actual_prey_pos and self.agent_pos == self.actual_pred_pos:
            ...
            font = pygame.font.Font('freesansbold.ttf', 32)
            text = font.render('Simulation Ended, Agent Died', True, self.green, self.transparent)
            textRect = text.get_rect()
            textRect.center = (self.display_width // 2, (self.display_height // 2) - 40 )
            self.screen.blit(text, textRect)
            ...
            font = pygame.font.Font('freesansbold.ttf', 32)
            text = font.render('Yellow Denotes Agent', True, self.yellow, self.transparent)
            textRect = text.get_rect()
            textRect.center = (self.display_width // 2, self.display_height // 2 )
            self.screen.blit(text, textRect)
            text = font.render('Violet Denotes Prey', True, self.violet, self.transparent)
            textRect = text.get_rect()
            textRect.center = (self.display_width // 2, (self.display_height // 2) + 40 )
            self.screen.blit(text, textRect)
            text = font.render('Red Denotes Predator', True, self.red, self.transparent)
            textRect = text.get_rect()
            textRect.center = (self.display_width // 2, (self.display_height // 2) + 80 )
            self.screen.blit(text, textRect)
            text = font.render('Green Denotes Catch', True, self.green, self.transparent)
            textRect = text.get_rect()
    ...
    def animation(self):
        ...
        moves_list = [self.agent_moves,self.actual_prey_moves, self.actual_pred_moves]
        len_list = [len(self.agent_moves),len(self.actual_prey_moves), len(self.actual_pred_moves)]
